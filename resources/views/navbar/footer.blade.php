<div class="container mx-auto px-4">
    <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
        <!-- Company Info -->
        <div>
            <div class="flex items-center space-x-4">
                <img src="{{asset('logo/Imari.jpg')}}" class="h-16 w-16 md:h-20 md:w-20" alt="Company Logo">
            </div>
            <p class="text-gray-300 mb-4">Your trusted partner for Imari real estate solutions.</p>
            
        </div>
        
        <!-- Quick Links -->
        <div>
            <h3 class="text-lg font-bold mb-4">Quick Links</h3>
            <ul class="space-y-2">
                <li><a href="/" class="text-gray-300 hover:text-orange-500 transition-colors duration-300">Home</a></li>
                <li><a href="/about_us" class="text-gray-300 hover:text-orange-500 transition-colors duration-300">About Us</a></li>
                <li><a href="/properties" class="text-gray-300 hover:text-orange-500 transition-colors duration-300">Properties</a></li>
                {{-- <li><a href="#" class="text-gray-300 hover:text-orange-500 transition-colors duration-300">Services</a></li> --}}
                <li><a href="/contact_us" class="text-gray-300 hover:text-orange-500 transition-colors duration-300">Contact</a></li>
            </ul>
        </div>
        
        <!-- Services -->
        <div>
            <h3 class="text-lg font-bold mb-4">Services</h3>
            <ul class="space-y-2">
                <li><a href="#" class="text-gray-300 hover:text-orange-500 transition-colors duration-300">Property Buying</a></li>
                <li><a href="#" class="text-gray-300 hover:text-orange-500 transition-colors duration-300">Property Selling</a></li>
                <li><a href="#" class="text-gray-300 hover:text-orange-500 transition-colors duration-300">Property Management</a></li>
                {{-- <li><a href="#" class="text-gray-300 hover:text-orange-500 transition-colors duration-300">Market Analysis</a></li> --}}
                {{-- <li><a href="#" class="text-gray-300 hover:text-orange-500 transition-colors duration-300">Investment Consulting</a></li> --}}
            </ul>
        </div>
        
        <!-- Contact Info -->
        <div>
            <h3 class="text-lg font-bold mb-4">Contact Us</h3>
            <ul class="space-y-2">
                <li class="flex items-start">
                    <i class="fas fa-map-marker-alt mt-1 mr-3 text-orange-500"></i>
                    <span> Ntarama ,Bugesera, Rwanda</span>
                </li>
                <li class="flex items-center">
                    <i class="fas fa-phone-alt mr-3 text-orange-500"></i>
                    <span>+250783578672</span>
                </li>
                <li class="flex items-center">
                    <i class="fas fa-envelope mr-3 text-orange-500"></i>
                    <span> emuhayimana@gmail.com</span>
                </li>
                <li class="flex items-center">
                    <i class="fas fa-clock mr-3 text-orange-500"></i>
                    <span>Mon-Fri: 8AM - 8PM</span>
                </li>
            </ul>
        </div>
    </div>
    
    <div class="border-t border-blue-800 mt-12 pt-6 text-center text-gray-400">
        <p>&copy; 2025 Imari Real Estate. All rights reserved.</p>
    </div>
</div>